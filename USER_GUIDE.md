...
When it starts, you should see a short **Ready** panel with the **data directory** (where the database lives) and the current **mode** (`local`, `cloud`, or `auto`).
...
| `/export` | Show where the database and tool workspace are on disk |
| `/quit` | Exit Waterloo |
...
- **Default folder:** `~/waterloo-ws` (under your home directory). It is created automatically if missing.
- **Override:** set `WATERLOO_TOOL_ROOT` to a different directory if you want.
...
1. Set **either**:
   - `WATERLOO_ICAL_PATH` to a file on disk, **or**
   - `WATERLOO_ICAL_URL` to an **`https://`** link that returns an `.ics` file.
...
| `WATERLOO_DATA_DIR` | Where `waterloo.db` and related data go |
| `WATERLOO_TOOL_ROOT` | Sandbox for `/read` and cwd for `/run` (default `~/waterloo-ws`) |
...
- **macOS:** `~/Library/Application Support/waterloo/waterloo.db`
- **Linux:** `~/.local/share/waterloo/waterloo.db`

Use `/export` inside Waterloo to print the exact paths on your machine.
...
| Wrong or missing files from `/read` | Paths are under `WATERLOO_TOOL_ROOT` (default `~/waterloo-ws`). Put files there or adjust the variable. |
